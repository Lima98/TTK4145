...
struct BoundedBuffer* buf_new(int size){
    ...
    sem_init(&buf->capacity,      0, size); //antar denne er size til å begynne med fordi buffer er tom
	sem_init(&buf->numElements,   0, 0); //antar at denne er 0 til å begynne med fordi buffer er tom
